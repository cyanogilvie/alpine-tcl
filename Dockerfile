...
FROM docker.io/library/amazonlinux:2023.11.20260514.0 AS src-al2023
RUN dnf upgrade -q -y
ENV LANG=C.UTF-8
...
ADD --unpack https://ftpmirror.gnu.org/gnu/autoconf/autoconf-2.71.tar.xz /src
WORKDIR /src/autoconf-2.71
RUN ./configure
RUN make -j install
# Amazon Linux 2023 >>>
# Ubuntu 24.04 <<<
...
ADD --unpack https://github.com/ninja-build/ninja/archive/refs/tags/v1.13.2.tar.gz /src
WORKDIR /src/ninja-1.13.2
RUN ./configure.py --bootstrap
#RUN ./ninja all
RUN cp ninja /out/usr/local/bin/

ADD --unpack https://github.com/mesonbuild/meson/archive/refs/tags/1.11.1.tar.gz /src
WORKDIR /src/meson-1.11.1
RUN ./packaging/create_zipapp.py --outfile /out/usr/local/bin/meson --interpreter '/usr/bin/env python3' .
# ninja and meson >>>
# Tcl bootstrap <<<
FROM base-build AS bootstrap-tcl
...
RUN mkdir -p /bootstrap/lib \
 && cat <<'EOF' > /bootstrap/lib/tclConfig.sh
TCL_VERSION="9.0"
TCL_MAJOR_VERSION="9"
TCL_MINOR_VERSION="0"
TCL_PATCH_LEVEL=".3"
TCL_LIB_SPEC="-L/bootstrap/lib -ltcl9.0"
TCL_INCLUDE_SPEC="-I/bootstrap/include"
TCL_SUPPORTS_STUBS=1
TCL_STUB_LIB_FILE="libtclstub.a"
TCL_STUB_LIB_FLAG="-ltclstub"
TCL_STUB_LIB_SPEC="-L/bootstrap/lib -ltclstub"
EOF
RUN mkdir -p /tmp/bootstrap/aws \
 && cd /tmp/bootstrap/aws \
 && git clone --recurse-submodules --shallow-submodules --branch v2.0a28 --single-branch --depth 1 https://github.com/cyanogilvie/aws-tcl . \
 && meson setup build --prefix=/bootstrap -Dtm_mode=ziplet --buildtype=release --strip \
 && meson install -C build \
 && mkdir -p /tmp/bootstrap/tclsignal \
 && cd /tmp/bootstrap/tclsignal \
 && git clone -b v1.4.5-cyan1 --recurse-submodules --shallow-submodules --single-branch --depth 1 https://github.com/cyanogilvie/tclsignal . \
 && autoconf && ./configure --with-tcl=/bootstrap/lib --prefix=/bootstrap \
 ...
 && rm -r /tmp/bootstrap
COPY common_sighandler-*.tm /bootstrap/lib/tcl9/site-tcl/
#>>>

# lambda-builder: base-build + builder tooling to build a stage in a lambda instance <<<
FROM bootstrap-tcl AS lambda-builder
...
WORKDIR /src/boost
RUN git clone https://github.com/boostorg/boost.git -b boost-1.91.0 --depth 1 .
...
ADD --unpack https://github.com/libexif/libexif/releases/download/v0.6.26/libexif-0.6.26.tar.gz /src
WORKDIR /src/libexif-0.6.26
...
ADD --unpack https://github.com/libjpeg-turbo/libjpeg-turbo/releases/download/3.1.4.1/libjpeg-turbo-3.1.4.1.tar.gz /src
WORKDIR /src/libjpeg-turbo-3.1.4.1/builddir
...
ADD --unpack https://zlib.net/zlib-1.3.2.tar.gz /src
WORKDIR /src/zlib-1.3.2
...
ADD --unpack https://storage.googleapis.com/downloads.webmproject.org/releases/webp/libwebp-1.6.0.tar.gz /src
WORKDIR /src/libwebp-1.6.0
...
ADD --unpack https://gitlab.com/libtiff/libtiff/-/archive/v4.7.1/libtiff-v4.7.1.tar.gz /src
...
ADD --unpack https://www.oberhumer.com/opensource/lzo/download/lzo-2.10.tar.gz /src
WORKDIR /src/lzo-2.10
RUN ./configure --disable-shared --with-pic
RUN make -j
RUN make install DESTDIR=/out
# lzo2 >>>
# libunwind (PIC static) <<<
# Alpine's distro libunwind-static ships /usr/lib/libunwind.a built WITHOUT
# -fPIC, so it can't be linked into the shared libtcl9Pixel_svg_cairo.so
# (librsvg pulls it in transitively): the link fails with "relocation
# R_X86_64_PC32 against `_ULx86_64_local_addr_space` ... recompile with -fPIC".
# We've dropped the distro package; build libunwind from source with -fPIC
# instead. It installs to the default /usr/local (a writable region under the
# read-only-rootfs Lambda runner, unlike /usr) and the stages that need it
# (base-build-rsvg, package-pixel-svg_cairo) COPY it in /out -> / like every
# other dep. librsvg's meson resolves rustc's `-lunwind` to this archive via
# cc.find_library() (gcc searches /usr/local/lib) and bakes that path into
# librsvg-2.0.pc.
FROM base-build AS base-build-libunwind
ADD --unpack https://github.com/libunwind/libunwind/releases/download/v1.8.3/libunwind-1.8.3.tar.gz /src
WORKDIR /src/libunwind-1.8.3
#RUN ./configure CFLAGS="${CFLAGS_ARCH}" LDFLAGS="${LDFLAGS_ARCH}" --prefix=/usr --disable-shared --enable-static --disable-minidebuginfo --disable-tests --disable-documentation
RUN ./configure CFLAGS="${CFLAGS_ARCH}" LDFLAGS="${LDFLAGS_ARCH}" --disable-shared --enable-static --disable-minidebuginfo --disable-tests --disable-documentation
RUN make -j
RUN make install DESTDIR=/out
# libunwind >>>
# gperf <<<
...
ADD --unpack https://github.com/libexpat/libexpat/releases/download/R_2_8_1/expat-2.8.1.tar.gz /src
WORKDIR /src/expat-2.8.1
...
# NOTE: held at 2.17.1 — fontconfig 2.18.0 moved the FcFreeTypeQuery /
# FcFreeTypeQueryAll prototypes out of <fontconfig/fontconfig.h> into
# <fontconfig/fcfreetype.h>. pango (1.56.4 and 1.57.0) only includes
# fontconfig.h in pangofc-fontmap.c, so building it against 2.18.0 fails
# with an implicit-declaration error on FcFreeTypeQueryAll (-Werror).
ADD --unpack https://gitlab.freedesktop.org/fontconfig/fontconfig/-/archive/2.17.1/fontconfig-2.17.1.tar.gz /src
WORKDIR /src/fontconfig-2.17.1
...
ADD --unpack https://github.com/libffi/libffi/releases/download/v3.5.2/libffi-3.5.2.tar.gz /src
WORKDIR /src/libffi-3.5.2
...
ADD --unpack https://download.gnome.org/sources/glib/2.86/glib-2.86.5.tar.xz /src
WORKDIR /src/glib-2.86.5
COPY --link --from=base-build-ninja				/out /
COPY --link --from=base-build-gettext			/out /
COPY --link --from=base-build-zlib				/out /
...
ADD --unpack https://cairographics.org/releases/pixman-0.46.4.tar.gz /src
WORKDIR /src/pixman-0.46.4
COPY --link --from=base-build-ninja				/out /
...
ADD --unpack https://gitlab.gnome.org/GNOME/libxml2/-/archive/v2.15.3/libxml2-v2.15.3.tar.gz /src
WORKDIR /src/libxml2-v2.15.3
COPY --link --from=base-build-ninja				/out /
COPY --link --from=base-build-zlib				/out /
...
ADD --unpack https://gitlab.freedesktop.org/xdg/shared-mime-info/-/archive/2.4/shared-mime-info-2.4.tar.gz /src
WORKDIR /src/shared-mime-info-2.4
COPY --link --from=base-build-gettext			/out /
...
ADD --unpack https://gitlab.gnome.org/GNOME/gdk-pixbuf/-/archive/2.44.6/gdk-pixbuf-2.44.6.tar.gz /src
WORKDIR /src/gdk-pixbuf-2.44.6
COPY --link --from=base-build-glib				/out /
COPY --link --from=base-build-jpeg-turbo		/out /
COPY --link --from=base-build-libffi			/out /
COPY --link --from=base-build-libpng			/out /
...
ADD --unpack https://github.com/harfbuzz/harfbuzz/releases/download/14.2.0/harfbuzz-14.2.0.tar.xz /src
WORKDIR /src/harfbuzz-14.2.0
COPY --link --from=base-build-ninja				/out /
COPY --link --from=base-build-freetype			/out /
COPY --link --from=base-build-glib				/out /
RUN meson setup builddir/ --buildtype release --default-library static --optimization 2 --prefer-static \
...
ADD --unpack https://github.com/fribidi/fribidi/releases/download/v1.0.16/fribidi-1.0.16.tar.xz /src
WORKDIR /src/fribidi-1.0.16
COPY --link --from=base-build-ninja		/out /
RUN meson setup build/ --buildtype release --default-library static --optimization 2 --prefer-static \
	-D docs=false \
	-D bin=false \
...
ADD --unpack https://cairographics.org/releases/cairo-1.18.4.tar.xz /src
WORKDIR /src/cairo-1.18.4
COPY --link --from=base-build-expat				/out /
...
ADD --unpack https://gitlab.gnome.org/GNOME/pango/-/archive/1.56.4/pango-1.56.4.tar.gz /src
WORKDIR /src/pango-1.56.4
...
#WORKDIR /src/rsvg
#RUN wget -q https://gitlab.gnome.org/GNOME/librsvg/-/archive/2.62.2/librsvg-2.62.2.tar.gz -O - | tar xz --strip-components=1
ADD --unpack https://gitlab.gnome.org/GNOME/librsvg/-/archive/2.62.2/librsvg-2.62.2.tar.gz /src
WORKDIR /src/librsvg-2.62.2
COPY --link --from=base-build-cairo				/out /
COPY --link --from=base-build-expat				/out /
COPY --link --from=base-build-fontconfig		/out /
COPY --link --from=base-build-freebidi			/out /
...
COPY --link --from=base-build-libpng			/out /
COPY --link --from=base-build-libtiff			/out /
# librsvg's meson resolves rustc's reported `-lunwind` via cc.find_library()
# and bakes the resulting path into librsvg-2.0.pc's Libs.private. In
# --prefer-static mode find_library scans the compiler's own lib dirs
# (`gcc -print-search-dirs`), which do NOT include /usr/local/lib — so the
# meson setup below sets LIBRARY_PATH=/usr/local/lib to add it (the bare
# `cc -lunwind` link works regardless because the *linker* searches there, but
# meson's static search doesn't). The distro lib worked before only because it
# sat in /usr/lib, which is already in that list.
COPY --link --from=base-build-libunwind			/out /
COPY --link --from=base-build-libxml2			/out /
COPY --link --from=base-build-lzo2				/out /
COPY --link --from=base-build-ninja				/out /
...
COPY --link --from=base-build-shared-mime-info	/out /
COPY --link --from=base-build-zlib				/out /
# LIBRARY_PATH adds /usr/local/lib to the compiler lib dirs meson scans when
# resolving the static `unwind` library (see the libunwind COPY note above).
...
COPY --link --from=base-build-ninja				/out /
ADD --unpack https://github.com/libtom/libtommath/releases/download/v1.3.0/ltm-1.3.0.tar.xz /src
WORKDIR /src/libtommath-1.3.0
...
WORKDIR /src/libtommath-1.3.0/build
...
# tcl: core-9-0-3 tag
ADD --unpack https://core.tcl-lang.org/tcl/tarball/core-9-0-3/tcl.tar.gz /src
WORKDIR /src/tcl
ARG SITETCL=/usr/local/lib/tcl9/site-tcl
# tclconfig: tip of main
ADD --unpack https://core.tcl-lang.org/tclconfig/tarball/6e82b0097c/tclconfig.tar.gz /src
WORKDIR /src/tclconfig
# thread: tip of main
ADD --unpack https://core.tcl-lang.org/thread/tarball/thread-3-0-5/thread.tar.gz /src
WORKDIR /src/thread
RUN ln -s ../tclconfig
# tcl9.0 >>>
# tcl8.7 <<<
...
# b1
ADD --unpack https://core.tcl-lang.org/tcl/tarball/62ad42bba8/tcl.tar.gz /src
WORKDIR /src/tcl
...
ADD --unpack https://core.tcl-lang.org/thread/tarball/586530a607/thread.tar.gz /src
WORKDIR /src/thread
RUN ln -s ../tclconfig
# tcl8.7 >>>
...
RUN strip --strip-debug --keep-file-symbols --keep-section-symbols /usr/local/lib/libtclstub.a
#RUN ln -s tclsh$TCLVER /usr/local/bin/tclsh
RUN mkdir -p $SITETCL

WORKDIR /src/thread
RUN autoconf
RUN ./configure CFLAGS="${CFLAGS_ARCH} -g" LDFLAGS="${LDFLAGS_ARCH}" --with-tcl=/usr/local/lib
RUN make -j
...
ADD --unpack https://github.com/cyanogilvie/tcl-tomcrypt/releases/download/v0.9.8/tomcrypt-v0.9.8.tar.gz /src
WORKDIR /src/tomcrypt-v0.9.8
RUN CXXFLAGS="" meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-tomcrypt >>>
# package-pgwire <<<
...
WORKDIR /src/pgwire
RUN git clone -b v3.0.0b29 --recurse-submodules --shallow-submodules --single-branch --depth 1 https://github.com/cyanogilvie/pgwire .
...
ADD --unpack https://github.com/cyanogilvie/reuri/releases/download/v0.16.5/reuri-0.16.5.tar.xz /src
WORKDIR /src/reuri-0.16.5
...
WORKDIR /src/rl_http
RUN git clone -b 1.26 --recurse-submodules --shallow-submodules --single-branch --depth 1 https://github.com/RubyLane/rl_http .
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-rl_http >>>
# libbrotli <<<
FROM base-build AS base-build-libbrotli
ADD --unpack https://github.com/google/brotli/archive/refs/tags/v1.2.0.tar.gz /src
WORKDIR /src/brotli-1.2.0/build
...
WORKDIR /src/brotli
RUN git clone -q -b v0.4.5 --recurse-submodules --shallow-submodules --single-branch --depth 1 https://github.com/cyanogilvie/tcl-brotli .
...
ADD --unpack https://sqlite.org/2026/sqlite-autoconf-3530100.tar.gz /src
WORKDIR /src/sqlite-autoconf-3530100/tea
#RUN ./configure CFLAGS="${CFLAGS_ARCH}" LDFLAGS="${LDFLAGS_ARCH}"
...
COPY --link --from=package-pixel-core			/out /
# PIC libunwind, pulled in transitively by librsvg-2.0.pc's Libs.private. The
# path was baked into the .pc by the base-build-rsvg stage; it must resolve to
# this same /usr/local/lib/libunwind.a here for the shared-object link.
COPY --link --from=base-build-libunwind			/out /
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
...
ADD --unpack https://github.com/google/gumbo-parser/archive/v0.10.1.tar.gz /src
WORKDIR /src/gumbo-parser-0.10.1
RUN ./autogen.sh
...
WORKDIR /src/tdom
RUN git clone -b cyan-0.9.6.11 --recurse-submodules --shallow-submodules --single-branch --depth 1 https://github.com/RubyLane/tdom .
...
ADD --unpack https://github.com/RubyLane/parse_args/releases/download/v0.6.3/parse_args-v0.6.3.tar.gz /src
WORKDIR /src/parse_args-v0.6.3
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-parse_args >>>
# package-rl_json <<<
...
ADD --unpack https://github.com/cyanogilvie/cbor/releases/download/v1.1/cbor-v1.1.tar.gz /src
WORKDIR /src/cbor-v1.1
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-cbor >>>
# package-hash <<<
...
ADD --unpack https://github.com/cyanogilvie/unix_sockets/releases/download/v0.7/unix_sockets0.7.tar.gz /src
WORKDIR /src/unix_sockets0.7
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-unix_sockets >>>
# package-tclreadline <<<
...
ADD --unpack https://github.com/cyanogilvie/tclreadline/releases/download/v2.4.1.cyan15/tclreadline2.4.1.tar.gz /src
WORKDIR /src/tclreadline2.4.1
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
...
ADD --unpack https://github.com/cyanogilvie/type/releases/download/v0.4/type-v0.4.tar.gz /src
WORKDIR /src/type-v0.4
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-type >>>
# package-filewatch <<<
FROM tcl-build-base AS package-filewatch
ADD --unpack https://github.com/cyanogilvie/filewatch/releases/download/v2.4/filewatch-v2.4.tar.gz /src
WORKDIR /src/filewatch-v2.4
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-filewatch >>>
# package-parsetcl <<<
...
ADD --unpack https://github.com/tcltk/tcllib/archive/refs/tags/tcllib-2-0.tar.gz /src
WORKDIR /src/tcllib-tcllib-2-0
RUN ./configure
RUN make DESTDIR=/out install-libraries install-applications clean
COPY --link --from=package-brotli		/out /
COPY --link --from=package-chantricks		/out /
COPY tools/compresstcl.tcl /tmp
COPY tools/ziptcllib.tcl /tmp
RUN tclsh /tmp/compresstcl.tcl /out/usr/local/lib/tcllib2.0
RUN tclsh /tmp/ziptcllib.tcl /out/usr/local/lib/tcllib2.0
...
RUN git clone --recurse-submodules --shallow-submodules --branch v1.1 --single-branch --depth 1 https://github.com/RubyLane/gc_class .
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-gc_class >>>
# package-tbuild <<<
...
ADD --unpack https://github.com/cyanogilvie/tdbc/archive/1f8b684.tar.gz /src
WORKDIR /src/tdbc-1f8b6842187a20ff424b52b14d523b639cd6e212
RUN ln -s ../tclconfig && \
...
ADD --unpack https://github.com/cyanogilvie/tcl-s2n/releases/download/v0.8.4/s2n-0.8.4.tar.gz /src
WORKDIR /src/s2n-0.8.4
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-s2n >>>
# package-sockopt <<<
FROM tcl-build-base AS package-sockopt
ADD --unpack https://github.com/cyanogilvie/sockopt/releases/download/v0.4/sockopt-v0.4.tar.gz /src
WORKDIR /src/sockopt-v0.4
RUN meson setup builddir --buildtype=release -Ddebug=true
RUN meson install -C builddir --destdir /out
# package-sockopt >>>
# package-tty <<<
...
RUN git clone --recurse-submodules --shallow-submodules --branch v2.0a28 --single-branch --depth 1 https://github.com/cyanogilvie/aws-tcl .
RUN ldconfig || true
...
ADD --unpack https://github.com/aklomp/base64/archive/9e8ed65048ff0f703fad3deb03bf66ac7f78a4d7.tar.gz /src
WORKDIR /src/base64-9e8ed65048ff0f703fad3deb03bf66ac7f78a4d7
...
RUN strip --strip-debug --keep-file-symbols --keep-section-symbols /out$TCLROOT/lib/libtclstub.a
#RUN rm -rf *.o *.zip *.vfs
...
RUN find /out$TCLROOT/src -type f \
	\! -name '*.c' \
	\! -name '*.h' \
	\! -name '*.cc' \
	\! -name '*.cpp' \
...
# then invoke `tools/build_apk.tcl` via a privileged-enough
...
# replayed under a read-only-rootfs runtime.
FROM bootstrap-tcl AS package-apk-prep
COPY --link --from=tcl-copy /out /tmp/apk-build/payload-root
RUN apk add --no-cache --update \
	abuild
COPY tools/build_apk.tcl /var/task/build_apk.tcl
# package-apk-prep >>>
...
RUN /bootstrap/bin/tclsh /var/task/build_apk.tcl
# package-apk >>>
...
COPY --link --from=package-pixel-src	/src /src
