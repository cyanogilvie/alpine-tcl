...
WORKDIR /src/pgwire
RUN git clone -b v3.0.0b23 --recurse-submodules --shallow-submodules --single-branch --depth 1 https://github.com/cyanogilvie/pgwire .
...
COPY --link --from=package-tcllib		/out /
COPY --link --from=package-chantricks	/out /
RUN git clone --recurse-submodules --shallow-submodules --branch v2.0a12 --single-branch --depth 1 https://github.com/cyanogilvie/aws-tcl .
RUN make DESTDIR=/out install
# package-aws >>>
# aklomp/base64 <<<
